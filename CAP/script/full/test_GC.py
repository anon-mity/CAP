...
def write_to_csv(file_path, category,  total_T_dis_mean, total_T_hcm_dis_mean):
    # 打开或创建一个 CSV 文件并附加写入数据
    with open(file_path, mode='a', newline='') as file:
        writer = csv.writer(file)
        # 写入一行数据
        writer.writerow([category,  total_T_dis_mean, total_T_hcm_dis_mean])

import torch


def rotate_point_cloud_torch(points, angle='90'):

    if angle == '90':
        rotation_matrix = torch.tensor([[0, 0, 1],
                                        [0, 1, 0],
                                        [-1, 0, 0]], device='cuda',  dtype=torch.float32)
    elif angle == '-90':
        rotation_matrix = torch.tensor([[0, 0, -1],
                                        [0, 1, 0],
                                        [1, 0, 0]],device='cuda', dtype=torch.float32)
        ...
        raise ValueError("角度必须是 '90', '-90', 或 '180'")
...
def test(conf, network, val_dataloader, category):
    ...
    for i, val_batch in val_batches:
        ...
        with torch.no_grad():
            ...
            for i in range(conf.exp.batch_size):
                ...
                #norm = norm_data[i]
                #Xc = inputx @ T0
                #img_path = f'/home/hanbing/paper_code/CAP/vis/PCA/{category}_GT_{i}.png'
                #visualize_point_clouds(Xc, norm, img_path)
                Xc =  rotate_point_cloud_torch(Xc, angle='90')

                dis_T = computeCD(Xc.unsqueeze(0), norm.unsqueeze(0))
                dis_T_hcm = computeCD(Xc.unsqueeze(0), norm.unsqueeze(0))

                distanceT[i] = dis_T
                distanceT_hcm[i] = dis_T_hcm
    ...
    cate = category
    CC_save_path = os.path.join(cfg.exp.log_dir, 'HPC.csv')
    write_to_csv(CC_save_path, cate, total_T_dis_mean, total_T_hcm_dis_mean)
